...
let roomId = RID
    window.joinRoomFirebase(roomId, function(success) {
        if (success) {
            ...
            window.listenToRoomFirebase(roomId, false, function(opponentData, status) {
                // Здесь можно обновлять EnemyBird
                rooms.switch('GameMultiplayer');
            });
        }
    });
...
rooms.templates['ui'] = {
    ...
    height: 720,
    behaviors: JSON.parse('[]'),
    objects: JSON.parse('[{"x":638.74683544,"y":389.24894515,"opacity":1,"tint":16777215,"scale":{"x":0.5021097,"y":0.5021097},"rotation":0,"exts":{},"customProperties":{},"template":"play"},{"x":1051.19605078,"y":376.75105485,"opacity":1,"tint":16777215,"scale":{"x":0.60789845,"y":0.4978903},"rotation":0,"exts":{},"customProperties":{},"template":"label2"},{"x":896,"y":356,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"customText":"...","customAnchor":{"x":0,"y":0},"template":"score"},{"x":228,"y":376,"opacity":1,"tint":"#fffb00","scale":{"x":0.60789845,"y":0.4978903},"rotation":0,"exts":{},"customProperties":{},"template":"skins"},{"x":638,"y":152,"opacity":1,"tint":16777215,"scale":{"x":0.6,"y":0.4},"rotation":0,"exts":{},"customProperties":{},"template":"gameover"},{"x":0,"y":0,"opacity":1,"tint":"#ffffff","scale":{"x":1.1962963,"y":1.05555556},"rotation":0,"exts":{},"customProperties":{},"template":"bg"},{"x":52.9957773,"y":584.99999977,"opacity":1,"tint":16777215,"scale":{"x":0.40903275,"y":0.41772152},"rotation":0,"exts":{},"customProperties":{},"customText":"Мультиплеер","customSize":"20","template":"multiplayerBtn"},{"x":626,"y":466,"opacity":1,"tint":16777215,"scale":{"x":1.34462149,"y":0.78571429},"rotation":0,"exts":{},"customProperties":{},"customText":"ЛУЧШИЕ ИГРОКИ\\n    Загрузка...","customWordWrap":"800","template":"leaderboardText"},{"x":26,"y":456,"opacity":1,"tint":"#6981ff","scale":{"x":0.5021097,"y":0.5021097},"rotation":0,"exts":{},"customProperties":{},"customText":"Имя?","template":"nameinput"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[]'),
    backgroundColor: '#000000',
...
adaptObject(multiplayerBtn, 52.9957773, 584.99999977, 0.55, 0.55);

// 5. Текст лидерборда (X:600, Y:466, Scale:1.534×0.786)
let leaderboardText = templates.list['leaderboardText']?.[0];
adaptObject(leaderboardText, 600, 466, 1.53386452, 0.78571429);
...
appScript.onload = function() {
    ...
    dbScript.onload = function() {
        ...
        window.loadLeaderboardFirebase = function(callback) {
            db.ref('scores').orderByChild('score').limitToLast(10).once('value')
                .then(snapshot => {
                    ...
                    scores.reverse(); // Сортируем по убыванию
        ...
        window.loadLeaderboardFirebase(function(scores) {
            let textElement = templates.list['leaderboardText']?.[0];
            if (textElement) {
                let text = '🏆 ЛУЧШИЕ ИГРОКИ\n━━━━━━━━━━━━━━━━━━\n';
                if (scores.length === 0) {
                    text += 'Пока нет рекордов...\nСтань первым! 🎮';
...
console.log('🔥 Запуск мультиплеерного меню (Firebase)...');

// ---- ФУНКЦИИ ОБНОВЛЕНИЯ UI ----
function updateStatus(text) {
    let statusText = templates.list['statusText']?.[0];
    if (statusText) statusText.text = 'Статус: ' + text;
}

function updateRoomId(text) {
    let roomIdDisplay = templates.list['roomIdDisplay']?.[0];
    if (roomIdDisplay) roomIdDisplay.text = 'Код: ' + text;
}

// // ---- КНОПКА "СОЗДАТЬ ИГРУ" (ХОСТ) ----
...
//         window.createRoomFirebase(function(roomId) {
//             updateStatus('✅ Комната создана! Ожидаем игрока...');
//             updateRoomId(roomId);
...
//             window.listenToRoomFirebase(roomId, true, function(opponentData, status) {
//                 if (status === 'playing') {
//                     updateStatus('✅ Игрок подключился! Запуск...');
//                     setTimeout(function() {
//                         rooms.switch('GameMultiplayer');
//                     }, 1000);
//                 }
...
// // ---- КНОПКА "ПОДКЛЮЧИТЬСЯ" (КЛИЕНТ) ----
let connectBtn = templates.list['connectBtn']?.[0];
// if (connectBtn) {
//     connectBtn.onClick = function() {
//         let roomIdInput = templates.list['roomIdInput']?.[0];
//         let roomId = roomIdInput ? roomIdInput.text : '';
        
//         if (!roomId) {
//             updateStatus('❌ Введите код комнаты!');
//             return;
//         }
...
//         window.joinRoomFirebase(roomId, function(success) {
//             if (success) {
//                 updateStatus('✅ Подключено! Запуск...');
//                 window._roomId = roomId;
//                 window._isHost = false;
                
//                 // Слушаем изменения в комнате
//                 window.listenToRoomFirebase(roomId, false, function(opponentData) {
//                     // Когда соперник двигается, обновляем его позицию
//                     // Это будет обрабатываться в игровой комнате
//                 });
...
// ============================================================
// АДАПТАЦИЯ ВСЕХ UI-ОБЪЕКТОВ (комната ui2) — ОСТАВЬТЕ КАК ЕСТЬ
// ============================================================
// (Скопируйте сюда существующий код адаптации из вашего проекта)
...
// 1. Заголовок (X:638, Y:152, Scale:0.6×0.4)
let title = templates.list['gameover']?.[0];
adaptObject(title, 638, 152, 0.6, 0.4);

// 2. Кнопка "Создать игру" (X:510, Y:293, Scale:0.39915374×0.64135021)
adaptObject(createBtn, 510, 293, 0.39915374, 0.64135021);

// 3. Кнопка "Подключиться" (X:130, Y:380, Scale:0.39915374×0.64135025)
adaptObject(connectBtn, 130, 380, 0.39915374, 0.64135025);

// 4. Кнопка "Назад" (X:499, Y:532, Scale:0.42877292×0.64135017)
adaptObject(backBtn, 499, 532, 0.42877292, 0.64135017);

// 5. Поле ввода ID (X:927, Y:380, Scale:0.39915374×0.64135021)
let roomIdInput = templates.list['roomIdInput']?.[0];
adaptObject(roomIdInput, 927, 380, 0.39915374, 0.64135021);

// 6. Отображение ID (X:836, Y:304, Scale:1×1)
let roomIdDisplay = templates.list['roomIdDisplay']?.[0];
adaptObject(roomIdDisplay, 836, 304, 1, 1);

// 7. Статус (X:304, Y:304, Scale:1×1)
let statusText = templates.list['statusText']?.[0];
adaptObject(statusText, 304, 304, 1, 1);
