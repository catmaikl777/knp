...
if (this.light) {
  ...
  var exportedSounds = [
    [{"name":"wing","variants":[{"uid":"bNtPFJNPqTLjdM","source":"./snd/bNtPFJNPqTLjdM.wav"},{"uid":"5rFM32zFtw6h67","source":"./snd/5rFM32zFtw6h67.wav"}],"preload":true,"panning":{"refDistance":0.5,"rolloffFactor":1},"distortion":{"enabled":true,"min":0.03,"max":1}},{"name":"hit","variants":[{"uid":"tMrzq4jB31pqq1","source":"./snd/tMrzq4jB31pqq1.wav"}],"preload":true,"panning":{"refDistance":0.5,"rolloffFactor":1},"volume":{"enabled":true,"min":0,"max":1}},{"name":"powerup","variants":[{"uid":"PmHcdRJb6nwkk9","source":"./snd/PmHcdRJb6nwkk9.mp3"}],"preload":true,"panning":{"refDistance":0.5,"rolloffFactor":1}},{"name":"point","variants":[{"uid":"CLkHN1hBHdNRh3","source":"./snd/CLkHN1hBHdNRh3.mp3"}],"preload":true,"panning":{"refDistance":0.5,"rolloffFactor":1},"volume":{"enabled":true,"min":1.11,"max":1.63}},{"name":"die","variants":[{"uid":"Lh7mhT3wWG8qjq","source":"./snd/Lh7mhT3wWG8qjq.mp3"}],"preload":true,"panning":{"refDistance":0.5,"rolloffFactor":1}},{"name":"coin","variants":[{"uid":"7QLqjN5kWGqghW","source":"./snd/7QLqjN5kWGqghW.wav"}],"preload":true,"panning":{"refDistance":0.5,"rolloffFactor":1},"volume":{"enabled":true,"min":0.5,"max":0.51}}]
  ][0] ?? [];
  var soundMap = {};
  ...
  var resLib = {
    ...
    async loadGame() {
      ...
      let atlases = [
        ["./img/a0.{webp,png}.ec133e7cda.json","./img/a1.{webp,png}.37ed3f639f.json","./img/a2.{webp,png}.a5df713cd2.json"]
      ][0];
      let bitmapFonts = [
        ["H3FrMjMCdb9FB2.xml"]
      ][0];
...
${ev.error?.stack ?? "(no stack available)"}`;
  ...
  var meta = [
    {"name":"Косатка не против полетать","author":"CATMAIKLGAMES","site":"","version":"1.0.0"}
  ][0];
...
inputs.addAction('AltPress', [{"code":"pointer.Secondary"},{"code":"pointer.Double"}]);
inputs.addAction('Scale', [{"code":"pointer.DeltaPinch"},{"code":"pointer.Wheel"}]);
inputs.addAction('touchPress', [{"code":"pointer.Any"}]);
inputs.addAction('NewAction', [{"code":"keyboard.Space"},{"code":"keyboard.ArrowUp"}]);

    
styles.new(
    "New Style",
    {
    "fontFamily": "OpenSans-VariableFont_wdth,wght, sans-serif",
    "fontSize": 32,
    "fontStyle": "normal",
    "fontWeight": "400",
    "align": "center",
...
templates.templates["default-player"] = {
    name: "default-player",
    depth: 0,
    ...
    onStep: function () {
        /* 🐱👉 template default-player — On frame start event (core_OnStep) */
{
...
if (((this.x >= camera.right) || (this.x <= camera.left))) {
    this.hspeed = (this.hspeed * -1);
this.scale.set((this.scale.x * -1), 1.2);
sounds.play('point', {
volume: 5
});
/* РЫБЫ */
templates.withTemplate('Coin', function () {
    this.kill = true;
});
templates.copy('Coin', 640, random.range((camera.top + 100), (camera.bottom - 205)));
templates.withTemplate('Coin', function () {
    this.scale.x = this.scale.y = 0.09;
});
/* ТРУБЫ */
// ============================================
// УПРАВЛЕНИЕ ТРУБАМИ — ЧЕРЕДОВАНИЕ ПАР
// ============================================

console.log('===== ОТСКОК! НАЧАЛО =====');

// 1. Находим все 4 трубы
let top1 = templates.list['pipe-up_2']?.[0];
let bottom1 = templates.list['pipe-bottom_2']?.[0];
let top2 = templates.list['pipe-up']?.[0];
let bottom2 = templates.list['pipe-bottom']?.[0];

if (!top1 || !bottom1 || !top2 || !bottom2) {
    console.log('❌ ОШИБКА: Не все трубы найдены!');
    return;
}

// Счётчик отскоков (хранится глобально)
if (typeof window._bounceCount === 'undefined') {
    window._bounceCount = 0;
}
window._bounceCount++;

let isMovingRight = this.speedX > 0;

// 2. Функция плавной анимации Y
function animateY(pipe, toY, pipeName, duration = 500) {
    if (!pipe) return;
    
    if (pipe._animYId) {
        try { cancelAnimationFrame(pipe._animYId); } catch(e) {}
        pipe._animYId = null;
    }
    
    if (Math.abs(pipe.y - toY) < 0.5) {
        console.log(`  ⏭ ${pipeName} уже на месте по Y (${pipe.y} ≈ ${toY})`);
        return;
    }
    
    console.log(`  🎬 ${pipeName}: Y ${Math.round(pipe.y)} → ${Math.round(toY)}`);
    
    let startY = pipe.y;
    let startTime = Date.now();
    pipe._isAnimatingY = true;
    
    function step() {
        if (!pipe || !pipe._isAnimatingY) return;
        
        let elapsed = Date.now() - startTime;
        let progress = Math.min(elapsed / duration, 1);
        let eased = 1 - Math.pow(1 - progress, 3);
        
        pipe.y = startY + (toY - startY) * eased;
        
        if (progress < 1) {
            pipe._animYId = requestAnimationFrame(step);
        } else {
            pipe.y = toY;
            pipe._isAnimatingY = false;
            pipe._animYId = null;
            console.log(`  ✅ ${pipeName}: Y анимация завершена, y = ${Math.round(pipe.y)}`);
        }
    }
...
// 3. Функция изменения Y пары труб
function scheduleYChange(pipeTop, pipeBottom, nameTop, nameBottom, delay = 1000) {
    if (!pipeTop || !pipeBottom) return;
    
    // ============================================
    // ПРОЁМ ВСЕГДА ОДИНАКОВЫЙ (с защитой)
    // ============================================

    let screenHeight = camera.height;
    let gapSize = 228;
    let topOffset = screenHeight * 0.15;
    let bottomOffset = screenHeight * 0.15;

    // Максимальное смещение — чтобы трубы не выходили за экран
    let maxOffset = screenHeight - topOffset - gapSize - bottomOffset - 20;

    // Случайное смещение (0, 64, 128) — но не больше maxOffset
    let gapOffset = Math.floor(Math.random() * 3) * 64;
    gapOffset = Math.min(gapOffset, maxOffset);

    // Позиции труб
    let newTopY = camera.top + topOffset + gapOffset - 154; // смещаем вверх на высоту трубы
    let newBottomY = newTopY + gapSize + 309;

    // Применяем
    // topPipe.y = newTopY;
    // bottomPipe.y = newBottomY;
    
    console.log(`  ⏳ Через ${delay/1000}с: ${nameTop} Y: ${Math.round(pipeTop.y)} → ${Math.round(newTopY)}, ${nameBottom} Y: ${Math.round(pipeBottom.y)} → ${Math.round(newBottomY)}`);
    
    setTimeout(() => {
        console.log(`  🔄 Запускаем анимацию Y для ${nameTop} и ${nameBottom}`);
        animateY(pipeTop, newTopY, nameTop, 500);
        animateY(pipeBottom, newBottomY, nameBottom, 500);
    }, delay);
}

// 4. ЧЕРЕДОВАНИЕ: чётные отскоки — Пара 1, нечётные — Пара 2
let isEvenBounce = window._bounceCount % 2 === 0;

console.log(`Отскок #${window._bounceCount}, ${isEvenBounce ? 'чётный' : 'нечётный'}`);
console.log(`Пара 1: x=${top1.x}, y=${top1.y}`);
console.log(`Пара 2: x=${top2.x}, y=${top2.y}`);

if (!isEvenBounce) {
    console.log('  Меняем Пара 1');
    scheduleYChange(top1, bottom1, 'top1', 'bottom1', 1000);
} else {
    console.log('  Меняем Пара 2');
    scheduleYChange(top2, bottom2, 'top2', 'bottom2', 1000);
}
...
/* РЕКОРД */
if (("HS" in localStorage)) {
    LdScr = localStorage.getItem("HS");
highScore = Number(LdScr);
...
if ((fish > highScore)) {
    highScore = fish;
localStorage.setItem("HS", String(fish));

}
/* БОНУСЫ */
// ============================================================
// БОНУСЫ — ПОЛНЫЙ КОД (Frame start)
// ============================================================
...
if (typeof this._hasDoubleCoins === 'undefined') {
    this._hasShield = false;
    this._hasDoubleCoins = false;
    this._hasSlowMo = false;
    ...
    this._magnetTimer = 0;
    this.canHurt = true;
    console.log('✅ Свойства бонусов созданы!');
}

// ============================================================
// 2. РЕГИСТРАЦИЯ ФУНКЦИЙ (один раз)
// ============================================================

if (!window._bonusFunctionsReady) {
    window._bonusFunctionsReady = true;
    console.log('📦 Регистрируем функции бонусов...');

    if (typeof window._targetSpeed === 'undefined') {
        window._targetSpeed = 1;
        window._currentSpeed = 1;
    }

    // ---------- АКТИВАЦИЯ ЩИТА ----------
    window.activateShield = function(bird) {
        if (!bird) return;
        console.log('🛡️ Щит активирован! (7 сек)');
        bird._hasShield = true;
        bird.canHurt = false;
        bird._shieldTimer = 70;
        bird.alpha = 0.7;
        if (sounds && sounds.play) sounds.play('powerup');
        window.showFloatingText(bird.x, bird.y - 60, '🛡️ Щит!', '#4444FF');
    };

    // ---------- АКТИВАЦИЯ ДВОЙНЫХ МОНЕТ ----------
    window.activateDoubleCoins = function(bird) {
        if (!bird) return;
        console.log('💰 Двойные монеты активированы! (10 сек)');
        bird._hasDoubleCoins = true;
        bird._doubleCoinsTimer = 100;
        if (sounds && sounds.play) sounds.play('powerup');
        window.showFloatingText(bird.x, bird.y - 60, '💰 x2!', '#FFD700');
    };

    // ---------- АКТИВАЦИЯ ЗАМЕДЛЕНИЯ ----------
    window.activateSlowMo = function(bird) {
        if (!bird) return;
        console.log('🐢 Замедление активировано! (7 сек)');
        bird._hasSlowMo = true;
        bird._slowMoTimer = 70;
        window._targetSpeed = 0.5;
        if (sounds && sounds.play) sounds.play('powerup');
        window.showFloatingText(bird.x, bird.y - 60, '🐢 Замедление!', '#AA44FF');
    };

    // ---------- АКТИВАЦИЯ МАГНИТА ----------
    window.activateMagnet = function(bird) {
        if (!bird) return;
        console.log('🧲 Магнит активирован! (12 сек)');
        bird._hasMagnet = true;
        bird._magnetTimer = 120;
        if (sounds && sounds.play) sounds.play('powerup');
        window.showFloatingText(bird.x, bird.y - 60, '🧲 Магнит!', '#FF4444');
    };

    // ---------- СБОР БОНУСА ----------
    window.collectBonus = function(bird, bonus) {
        if (!bird || !bonus) return;
        console.log('🎁 Собран бонус:', bonus.template);
        let templateName = bonus.template;
        switch(templateName) {
            case 'BonusShield': window.activateShield(bird); break;
            case 'BonusDouble': window.activateDoubleCoins(bird); break;
            case 'BonusSlowMo': window.activateSlowMo(bird); break;
            case 'BonusMagnet': window.activateMagnet(bird); break;
            default: return;
        }
        if (sounds && sounds.play) sounds.play('powerup');
        bonus.kill = true;
    ...
    // ---------- СБОР МОНЕТЫ (с x2) ----------
    window.collectCoin = function(bird, coin) {
        if (!bird || !coin) return;
        
        let isDouble = bird._hasDoubleCoins === true;
        let points = isDouble ? 2 : 1;
        
        console.log(`🪙 Монета: +${points} очков (x2=${isDouble})`);
        
        fish += points;
        
        if (isDouble) {
            window.showFloatingText(coin.x, coin.y - 30, '+2', '#FFD700', 0.8);
        }
        
        if (sounds && sounds.play) sounds.play('coin');
        coin.kill = true;
    };

    // ---------- ВСПЛЫВАЮЩИЙ ТЕКСТ ----------
    window.showFloatingText = function(x, y, text, color = '#FFFFFF', duration = 1.5) {
        if (!templates.list['FloatingText']) return;
        let label = templates.copy('FloatingText', x, y);
        if (!label) return;
        label.text = text;
        label.color = color;
        label.life = duration;
    ...
    console.log('✅ Функции бонусов зарегистрированы!');
}

// ============================================================
// 3. ПЛАВНАЯ СМЕНА СКОРОСТИ
// ============================================================

function updateSpeed() {
    let speed = 0.02;
    window._currentSpeed += (window._targetSpeed - window._currentSpeed) * speed;
    pixiApp.ticker.speed = window._currentSpeed;
    
    if (!window._hasSlowMo && Math.abs(window._currentSpeed - 1) < 0.01) {
        window._currentSpeed = 1;
...
function spawnBonuses() {
    ...
    if (!templates.list[templateName]) return;
    
    let bird = templates.list['Bird']?.[0] || window._bird;
    if (!bird) return;
    ...
    let x, y, dist;
    do {
        x = camera.left + margin + Math.random() * (camera.width - margin * 2);
        y = camera.top + margin + Math.random() * (camera.height - margin * 2);
        let dx = bird.x - x;
        let dy = bird.y - y;
        dist = Math.sqrt(dx * dx + dy * dy);
        attempts++;
    } while (dist < minDist && attempts < maxAttempts);
    if (dist < minDist) return;
    
    let bonus = templates.copy(templateName, x, y);
    if (!bonus) return;
    bonus.born = u.time;
    bonus.lifetime = 8 + Math.random() * 4;
    bonus._offset = Math.random() * 10;
}

// ============================================================
// 5. ОБНОВЛЕНИЕ БОНУСОВ НА СЦЕНЕ
// ============================================================

function updateBonuses() {
    let allBonuses = [
        ...(templates.list['BonusShield'] || []),
        ...(templates.list['BonusDouble'] || []),
        ...(templates.list['BonusSlowMo'] || []),
        ...(templates.list['BonusMagnet'] || [])
    ];
    for (let i = allBonuses.length - 1; i >= 0; i--) {
        let bonus = allBonuses[i];
        if (!bonus) continue;
        if (bonus.born === undefined) { bonus.kill = true; continue; }
        bonus.rotation += u.delta * 0.03;
        bonus.y += Math.sin(u.time * 3 + (bonus._offset || 0)) * 0.5;
        if (u.time - bonus.born > bonus.lifetime) {
            bonus.kill = true;
            continue;
        }
...
// ============================================================
// 6. ОБНОВЛЕНИЕ ЭФФЕКТОВ ПТИЦЫ
// ============================================================

function updateBirdBonuses(bird) {
    if (!bird) return;
    let delta = Math.min(u.delta, 0.18);
    
    // ---- Щит ----
    if (bird._hasShield) {
        bird._shieldTimer -= delta;
        bird.alpha = bird._shieldTimer < 1 && Math.floor(bird._shieldTimer * 10) % 2 === 0 ? 0.3 : 0.7;
        if (bird._shieldTimer <= 0) {
            bird._hasShield = false;
            bird.canHurt = true;
            bird.alpha = 1;
            console.log('❌ Щит исчез');
            window.showFloatingText(bird.x, bird.y - 70, 'Щит исчез', '#FF4444');
        }
    }
    
    // ---- Двойные монеты ----
    if (bird._hasDoubleCoins) {
        bird._doubleCoinsTimer -= delta;
        if (bird._doubleCoinsTimer <= 0) {
            bird._hasDoubleCoins = false;
            console.log('❌ x2 закончился');
            window.showFloatingText(bird.x, bird.y - 70, 'x2 закончился', '#FF4444');
        }
    }
    
    // ---- Замедление ----
    if (bird._hasSlowMo) {
        bird._slowMoTimer -= delta;
        if (bird._slowMoTimer <= 0) {
            bird._hasSlowMo = false;
            window._targetSpeed = 1;
            console.log('❌ Замедление закончилось');
            window.showFloatingText(bird.x, bird.y - 70, 'Скорость восстановлена', '#FF4444');
        }
    }
    
    // ---- Магнит ----
    if (bird._hasMagnet) {
        bird._magnetTimer -= delta;
        if (bird._magnetTimer <= 0) {
            bird._hasMagnet = false;
            console.log('❌ Магнит исчез');
            window.showFloatingText(bird.x, bird.y - 70, 'Магнит исчез', '#FF4444');
        }
    }
}

// ============================================================
// 7. МАГНИТ (притягивает монеты)
// ============================================================

function updateMagnet(bird) {
    if (!bird || !bird._hasMagnet) return;
    
    let delta = Math.min(u.delta, 0.1);
    let coins = templates.list['Coin'] || [];
    ...
    for (let i = 0; i < coins.length; i++) {
        ...
        if (dist < 300 && dist > 5) {
            let speed = 400 / (dist + 50);
            coin.x += (dx / dist) * speed * delta * 60;
            coin.y += (dy / dist) * speed * delta * 60;
            coin.alpha = 0.8;
        }
    }
}
...
function checkBonusCollisions(bird) {
    if (!bird || bird.gameover) return;
    let allBonuses = [
        ...(templates.list['BonusShield'] || []),
        ...(templates.list['BonusDouble'] || []),
        ...(templates.list['BonusSlowMo'] || []),
    ...
    for (let i = allBonuses.length - 1; i >= 0; i--) {
        let bonus = allBonuses[i];
        if (!bonus) continue;
        let dx = bird.x - bonus.x;
        let dy = bird.y - bonus.y;
        let dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < 40) {
            window.collectBonus(bird, bonus);
        }
    }
}

function checkCoinCollisions(bird) {
    if (!bird || bird.gameover) return;
    let coins = templates.list['Coin'] || [];
...
if (!this.gameover) {
    spawnBonuses();
    updateBonuses();
    updateBirdBonuses(this);
    updateMagnet(this);
    checkBonusCollisions(this);
    checkCoinCollisions(this);
    updateSpeed();
}

}
/* 🐱👉 template default-player — Collision with a group event (place_collisionCGroup) */
{
    const other = place.occupied(this, 'Obstacle');
...
transition.circleIn(0.5, 0)
    ...
    onCreate: function () {
        /* 🐱👉 template default-player — On create event (core_OnCreate) */
{
...
// ============================================================
// ГЛОБАЛЬНЫЕ ФУНКЦИИ ДЛЯ РАБОТЫ С ЛИДЕРБОРДОМ
// ============================================================

// Отправка рекорда
window.sendScore = function(playerName) {
    let score = highScore || 0;  // highScore - глобальная переменная
    
    if (score <= 0) {
        console.log('⚠️ Рекорд не отправлен (0 или меньше)');
        return;
    }
    ...
    fetch('https://simpleboards.dev/api/score', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            apiKey: 'faba1c34-5b45-452f-9b98-552d1365d86b',   // ЗАМЕНИТЕ НА ВАШ КЛЮЧ
            board: 'flappy_bird',
            player: playerName,
    ...
    .then(data => {
        console.log('✅ Рекорд отправлен!', data);
    })
    .catch(error => {
        console.log('❌ Ошибка отправки рекорда:', error);
    });
};

// Загрузка лидерборда
window.loadLeaderboard = function(callback) {
    fetch('https://simpleboards.dev/api/board/flappy_bird/top?limit=10')
        .then(response => response.json())
        .then(data => {
...
// Отображение лидерборда в UI-элементе
window.displayLeaderboard = function(scores, element) {
    if (!element) {
        console.warn('⚠️ UI-элемент для лидерборда не передан');
        return;
    }
    let text = '🏆 ЛУЧШИЕ ИГРОКИ\n━━━━━━━━━━━━━━━━━━\n';
    if (!scores || scores.length === 0) {
        text += 'Пока нет рекордов...\nСтань первым! 🎮';
    } else {
        scores.forEach((entry, index) => {
            let medal = ['🥇', '🥈', '🥉'][index] || (index + 1) + '.';
            text += medal + ' ' + (entry.player || 'Аноним') + ': ' + entry.score + '\n';
        });
    }
    element.text = text;
};
...
templates.templates["ground"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template ground — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template ground — On create event (core_OnCreate) */
{
emitters.fire('New Emitter Tandem', 640, 360, undefined);
u.wait(270)
                    .then(() => {
...
templates.templates["pipe-bottom"] = {
    name: "pipe-bottom",
    depth: 1,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template pipe-bottom — On frame start event (core_OnStep) */
{
let prog;
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template pipe-bottom — On create event (core_OnCreate) */
{
this.gravity = 0;
this.vspeed = 0;
u.wait(270)
                    .then(() => {
                        this.x = (camera.left + 76);
                    }).catch(() => {
...
templates.list['pipe-bottom'] = [];
        
templates.templates["Coin"] = {
    name: "Coin",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template Coin — On frame start event (core_OnStep) */
{
this.move();
if ((this.timer1 === 0)) {
    this.kill = true;
    ...
    onCreate: function () {
        /* 🐱👉 template Coin — On create event (core_OnCreate) */
{
this.timer1 = 4;
}
...
templates.templates["gameover"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template gameover — On frame start event (core_OnStep) */
{
this.move();
}
...
templates.templates["Placeholder"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template Placeholder — On frame start event (core_OnStep) */
{
this.move();
}
...
templates.templates["score"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template score — On frame start event (core_OnStep) */
{
this.move();
this.text = ("Рыбы " + String(fish) + ("    Рекорд " + String(highScore)));
if ((rooms.current === rooms.list['Game'])) {
    this.x = (camera.left + 249);
this.y = (camera.top + 17);
...
templates.list['score'] = [];
        
templates.templates["coin_2"] = {
    name: "coin_2",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template coin_2 — On frame start event (core_OnStep) */
{
this.move();
if ((rooms.current === rooms.list['Game'])) {
    u.wait(270)
                    .then(() => {
                        this.x = (camera.left + 193);
this.y = (camera.top + 38);
...
templates.list['coin_2'] = [];
        
templates.templates["pipe-up"] = {
    name: "pipe-up",
    depth: 1,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template pipe-up — On frame start event (core_OnStep) */
{
let prog;
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template pipe-up — On create event (core_OnCreate) */
{
u.wait(270)
                    .then(() => {
                        this.x = (camera.left + 76);
...
templates.list['pipe-up'] = [];
        
templates.templates["pipe-bottom_2"] = {
    name: "pipe-bottom_2",
    depth: 1,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template pipe-bottom_2 — On frame start event (core_OnStep) */
{
let prog;
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template pipe-bottom_2 — On create event (core_OnCreate) */
{
u.wait(270)
                    .then(() => {
                        this.x = (camera.right - 76);
                    }).catch(() => {
...
templates.list['pipe-bottom_2'] = [];
        
templates.templates["pipe-up_2"] = {
    name: "pipe-up_2",
    depth: 1,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template pipe-up_2 — On frame start event (core_OnStep) */
{
let prog;
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template pipe-up_2 — On create event (core_OnCreate) */
{
u.wait(270)
                    .then(() => {
                        this.x = (camera.right - 76);
...
templates.templates["HS"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template HS — On frame start event (core_OnStep) */
{
this.move();
u.wait(150)
                    .then(() => {
                        this.text = String(highScore);
                    }).catch(() => {
...
templates.templates["BonusSlowMo"] = {
    name: "BonusSlowMo",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template BonusSlowMo — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template BonusSlowMo — On create event (core_OnCreate) */
{
this.width = 64;
this.height = 64;
...
templates.templates["BonusShield"] = {
    name: "BonusShield",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template BonusShield — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template BonusShield — On create event (core_OnCreate) */
{
this.width = 64;
this.height = 64;
...
templates.templates["BonusMagnet"] = {
    name: "BonusMagnet",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template BonusMagnet — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template BonusMagnet — On create event (core_OnCreate) */
{
this.width = 64;
this.height = 64;
...
templates.templates["BonusDouble"] = {
    name: "BonusDouble",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template BonusDouble — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template BonusDouble — On create event (core_OnCreate) */
{
this.width = 64;
this.height = 64;
...
templates.templates["bg"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template bg — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template bg — On create event (core_OnCreate) */
{
u.wait(500)
                    .then(() => {
                        this.x = camera.left;
...
templates.templates["FloatingText"] = {
    name: "FloatingText",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template FloatingText — On frame start event (core_OnStep) */
{
this.move();
// ============================================================
// FloatingText — Frame start (без Draw)
// ============================================================

// Обновляем время жизни
this.life -= u.delta/10;
this.y = this.startY - (1 - this.life) * 60;
this.alpha = Math.min(this.life, 1);

// Создаём текст при первом кадре
if (!this._textCreated && this.text) {
    this._textCreated = true;
    
    // Правильный способ создания стиля
    let style = new PIXI.TextStyle({
        fontSize: 28,
        fill: this.color || '#FFFFFF',
        fontWeight: 'bold',
        stroke: '#000000',
        strokeThickness: 4,
        align: 'center',
    ...
    let label = new PIXI.Text(this.text, style);
    label.anchor.set(0.5, 0.5);
    this.addChild(label);
}

// Удаляем, когда время жизни закончилось
if (this.life <= 0) {
    this.kill = true;
}
    ...
    onCreate: function () {
        /* 🐱👉 template FloatingText — On create event (core_OnCreate) */
{
this['text'] = " ";
this['color'] = "#FFFFFF";
this['life'] = 15;
...
templates.templates["play"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template play — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template play — OnPointerClick event (core_OnPointerClick) */
this.eventMode = 'dynamic';
this.on('pointertap', () => {
...
templates.templates["skins"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template skins — On frame start event (core_OnStep) */
{
this.move();
}
...
templates.list['skins'] = [];
        
templates.templates["label2"] = {
    name: "label2",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template label2 — On frame start event (core_OnStep) */
{
this.move();
}
...
templates.templates["platform"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template platform — On frame start event (core_OnStep) */
{
this.move();
if ((gameover === false)) {
    this.x = (this.x + 7);
...
/* 🐱👉 template platform — Timer 1 event (core_Timer1) */
...
this.timer1 = 2;
    ...
    onCreate: function () {
        /* 🐱👉 template platform — On create event (core_OnCreate) */
{
this.angle = 90;
this.timer1 = 2;
...
templates.templates["saw"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template saw — On frame start event (core_OnStep) */
{
this.move();
}
...
templates.templates["EnemyBird"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template EnemyBird — On frame start event (core_OnStep) */
{
let spawnX, gapOffset, pipeOffset;
this.move();
// ============================================================
// ВРАЖЕСКАЯ ПТИЦА (синхронизируется по сети)
// ============================================================

// Плавное движение к целевой позиции
if (this._targetX !== undefined) {
    this.x += (this._targetX - this.x) * 0.1;
    this.y += (this._targetY - this.y) * 0.1;
}
// Визуальная анимация
this.rotation = Math.sin(u.time * 3) * 0.1;
// Счёт врага
if (this._targetScore !== undefined) {
    this.fish = this._targetScore;
}
// Если враг мёртв — затемняем
if (this.gameover) {
    this.alpha = 0.3;
}
...
templates.list['EnemyBird'] = [];
        
templates.templates["roomIdDisplay"] = {
    name: "roomIdDisplay",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template roomIdDisplay — On frame start event (core_OnStep) */
{
this.move();
}
...
templates.list['roomIdDisplay'] = [];
        
templates.templates["statusText"] = {
    name: "statusText",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template statusText — On frame start event (core_OnStep) */
{
this.move();
}
...
templates.list['statusText'] = [];
        
templates.templates["createBtn"] = {
    name: "createBtn",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.SCREEN,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template createBtn — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template createBtn — OnPointerClick event (core_OnPointerClick) */
this.eventMode = 'dynamic';
this.on('pointertap', () => {
    
window.createRoomHandler();
...
templates.list['createBtn'] = [];
        
templates.templates["connectBtn"] = {
    name: "connectBtn",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.SCREEN,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template connectBtn — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template connectBtn — OnPointerClick event (core_OnPointerClick) */
this.eventMode = 'dynamic';
this.on('pointertap', () => {
    
window.joinRoomHandler();
...
templates.list['connectBtn'] = [];
        
templates.templates["backBtn"] = {
    name: "backBtn",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.SCREEN,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template backBtn — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template backBtn — OnPointerClick event (core_OnPointerClick) */
this.eventMode = 'dynamic';
this.on('pointertap', () => {
...
templates.list['backBtn'] = [];
        
templates.templates["roomIdInput"] = {
    name: "roomIdInput",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template roomIdInput — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template roomIdInput — OnTextChange event (core_OnTextChange) */
...
templates.list['roomIdInput'] = [];
        
templates.templates["multiplayerBtn"] = {
    name: "multiplayerBtn",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template multiplayerBtn — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template multiplayerBtn — OnPointerClick event (core_OnPointerClick) */
this.eventMode = 'dynamic';
this.on('pointertap', () => {
    
rooms.switch('MultiplayerMenu');
...
templates.list['multiplayerBtn'] = [];
        
templates.templates["leaderboardText"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template leaderboardText — On frame start event (core_OnStep) */
{
this.move();
}
...
templates.list['leaderboardText'] = [];
        
templates.templates["nameinput"] = {
    name: "nameinput",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template nameinput — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template nameinput — OnTextChange event (core_OnTextChange) */
...
N = String(value);
localStorage.setItem("NAME", String(N));


}).bind(this);
/* 🐱👉 template nameinput — On create event (core_OnCreate) */
{
// Поиск текстового объекта среди детей
let textObj = null;
for (let i = 0; i < this.children.length; i++) {
    if (this.children[i] instanceof PIXI.Text || this.children[i] instanceof PIXI.BitmapText) {
        textObj = this.children[i];
        break;
    }
}

if (textObj) {
    let styleAsset = styles.get('New Style');
    if (styleAsset) {
        textObj.style = styleAsset;
        textObj.text = this.text || '';
    } else {
        textObj.style.fontFamily = 'Arial, sans-serif';
        textObj.style.fontSize = 24;
        textObj.style.fill = '#FFFFFF';
        textObj.style.stroke = '#000000';
...
templates.list['nameinput'] = [];
        
templates.templates["VT"] = {
    name: "VT",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template VT — On frame start event (core_OnStep) */
{
this.move();
}
...
templates.templates["toplist"] = {
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template toplist — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template toplist — OnPointerClick event (core_OnPointerClick) */
this.eventMode = 'dynamic';
this.on('pointertap', () => {
    
rooms.switch('toplistRoom');
...
templates.list['toplist'] = [];
        
templates.templates["backBtn2"] = {
    name: "backBtn2",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.SCREEN,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* 🐱👉 template backBtn2 — On frame start event (core_OnStep) */
{
this.move();
}
    ...
    onCreate: function () {
        /* 🐱👉 template backBtn2 — OnPointerClick event (core_OnPointerClick) */
this.eventMode = 'dynamic';
this.on('pointertap', () => {
...
rooms.templates['Game'] = {
    ...
    onStep() {
        /* 🐱👉 room Game — Timer 2 event (core_Timer2) */

if (this.timer2 > 0 && this.timer2 <= (false ? u.timeUi : u.time)) {
    this.timer2 = 0;
    
templates.withTemplate('default-player', function () {
    jf = Math.min((jf + 10), -150);
});
this.timer2 = 90;
...
this.timer2 = 100;
fish = 0;
templates.withTemplate('bg', function () {
    u.wait(270)
                    .then(() => {
                        this.x = camera.left;
this.y = camera.top;
...
rooms.templates['gameover'] = {
    name: 'gameover',
    width: 1280,
    height: 720,
    behaviors: JSON.parse('[]'),
    objects: JSON.parse('[{"x":0,"y":0,"opacity":1,"tint":16777215,"scale":{"x":1.18519,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"bg"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[]'),
    backgroundColor: '#000000',
    ...
    onCreate() {
        /* 🐱👉 room gameover — On room start event (core_OnRoomStart) */
{
templates.withTemplate('bg', function () {
    u.wait(400)
                    .then(() => {
...
rooms.templates['ui'] = {
    ...
    height: 720,
    behaviors: JSON.parse('[]'),
    objects: JSON.parse('[{"x":638.74683544,"y":389.24894515,"opacity":1,"tint":16777215,"scale":{"x":0.5021097,"y":0.5021097},"rotation":0,"exts":{},"customProperties":{},"template":"play"},{"x":595.19605078,"y":528.75105485,"opacity":1,"tint":16777215,"scale":{"x":0.60789845,"y":0.4978903},"rotation":0,"exts":{},"customProperties":{},"template":"label2"},{"x":440,"y":508,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"customText":"...","customAnchor":{"x":0,"y":0},"template":"score"},{"x":228,"y":376,"opacity":1,"tint":"#fffb00","scale":{"x":0.60789845,"y":0.4978903},"rotation":0,"exts":{},"customProperties":{},"template":"skins"},{"x":410,"y":152,"opacity":1,"tint":16777215,"scale":{"x":0.6,"y":0.4},"rotation":0,"exts":{},"customProperties":{},"template":"gameover"},{"x":0,"y":0,"opacity":1,"tint":"#ffffff","scale":{"x":1.1962963,"y":1.05555556},"rotation":0,"exts":{},"customProperties":{},"template":"bg"},{"x":52.9957773,"y":584.99999977,"opacity":1,"tint":16777215,"scale":{"x":0.40903275,"y":0.41772152},"rotation":0,"exts":{},"customProperties":{},"customText":"Мультиплеер","customSize":"20","template":"multiplayerBtn"},{"x":26,"y":456,"opacity":1,"tint":16777215,"scale":{"x":0.5021097,"y":0.5021097},"rotation":0,"exts":{},"customProperties":{},"customText":"Имя?","template":"nameinput"},{"x":380,"y":597,"opacity":1,"tint":"#ffc400","scale":{"x":0.502,"y":0.502},"rotation":0,"exts":{},"customProperties":{},"customText":"Топ Игроков","customSize":"28","template":"toplist"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[]'),
    backgroundColor: '#000000',
...
u.wait(770)
                    .then(() => {
                        // ============================================================
// АДАПТАЦИЯ ВСЕХ UI-ОБЪЕКТОВ ПО ИМЕНАМ ШАБЛОНОВ
// ============================================================

// В событии "Start" комнаты UIRoom

// ---- 1. РАЗМЕРЫ ЭКРАНА ----
let screenW = camera.width;
let screenH = camera.height;
let baseW = 1280;
let baseH = 720;
let scaleX = screenW / baseW;
let scaleY = screenH / baseH;

// ---- 2. ФУНКЦИЯ АДАПТАЦИИ ----
function adaptObject(obj, baseX, baseY, baseScaleX = 1, baseScaleY = 1, tint = null) {
    if (!obj) return;
    obj.x = baseX * scaleX;
    obj.y = baseY * scaleY;
    obj.scale.x = baseScaleX * scaleX;
    obj.scale.y = baseScaleY * scaleY;
    if (tint !== null) {
        obj.tint = tint;
    }
}

// ---- ФОН НА ВЕСЬ ЭКРАН через scale ----
let bg = templates.list['bg']?.[0] || this.getUI('bg');
if (bg) {
    bg.anchor.x = 0;
    bg.anchor.y = 0;
    bg.x = 0;
    bg.y = 0;
    bg.scale.x = screenW / bg.texture.width;
    bg.scale.y = screenH / bg.texture.height;
}

// ---- 3. АДАПТАЦИЯ ВСЕХ ОБЪЕКТОВ ПО СКРИНШОТАМ ----

// 1. Заголовок "Косатка не против полетать"
// X: 638, Y: 152, Scale: 0.6 x 0.4
let title = templates.list['gameover']?.[0] || this.getUI('gameover');
adaptObject(title, 638, 152, 0.6, 0.4);

// 2. Кнопка "Играть"
// X: 228, Y: 376, Scale: 0.608 x 0.498
let playBtn = templates.list['play']?.[0] || this.getUI('play');
adaptObject(playBtn, 638, 389, 0.502, 0.502);

// // 3. Кнопка "Выход"
// // X: 1051, Y: 376, Scale: 0.608 x 0.498, Tint: #FFFB80
// let exitBtn = templates.list['exit']?.[0] || this.getUI('exit');
// adaptObject(exitBtn, 638, 532, 0.502, 0.502, 0xFFFB80);

// 4. Кнопка "Скины"
// X: 638, Y: 532, Scale: 0.502 x 0.502
let skinsBtn = templates.list['skins']?.[0] || this.getUI('skins');
adaptObject(skinsBtn, 228, 376, 0.607, 0.497);

// 4. Кнопка "Мультиплеер" (X:53, Y:585, Scale:0.409×0.418)
let multiplayerBtn = templates.list['multiplayerBtn']?.[0];
adaptObject(multiplayerBtn, 52.9957773, 584.99999977, 0.55 * 2, 0.55 * 2.5);

// 5. Текст лидерборда (X:600, Y:466, Scale:1.534×0.786)
// let leaderboardText = templates.list['leaderboardText']?.[0];
// adaptObject(leaderboardText, 846, 10, 0,8243867, 0.757);

// 6. Поле ввода имени (X:26, Y:456, Scale:0.502×0.502)
let nameInput = templates.list['nameinput']?.[0];
adaptObject(nameInput, 26, 456, 0.65, 0.65);

// 6. label2 (текст)
// X: 1051, Y: 376, Scale: 0.608 x 0.498, Tint: #FFFB00
let label2 = templates.list['label2']?.[0] || this.getUI('label2');
adaptObject(label2, 595.19605078, 528.75105485, 0.608, 0.498, 0xFFFB00);

// 7. Score (текст счёта) — та же позиция, что и label2
let scoreText = templates.list['score']?.[0] || this.getUI('score');
adaptObject(scoreText, 440, 508, 1, 1);

let topBtn = templates.list['toplist']?.[0] || this.getUI("toplist");
adaptObject(topBtn, 380, 597, 0.502 * 2, 0.502 * 2);

// ============================================================
// ПОДКЛЮЧЕНИЕ FIREBASE (БЕЗ КАТМОДА)
// ============================================================

// 1. Загружаем скрипты с defer
let appScript = document.createElement('script');
appScript.defer = true;
appScript.src = 'https://www.gstatic.com/firebasejs/9.23.0/firebase-app-compat.js';
document.head.appendChild(appScript);

// 2. Когда App загрузился, грузим Database
appScript.onload = function() {
    ...
    // 3. Когда оба загружены, инициализируем Firebase
    dbScript.onload = function() {
        // ===== КОНФИГУРАЦИЯ (ЗАМЕНИТЕ НА СВОЮ!) =====
        const firebaseConfig = {
            apiKey: "AIzaSyB_TohbXZMytay40kIRmG6biLRH7qnlEfA",
            authDomain: "knpp-a5978.firebaseapp.com",
            databaseURL: "https://knpp-a5978-default-rtdb.europe-west1.firebasedatabase.app",
            projectId: "knpp-a5978",
            storageBucket: "knpp-a5978.firebasestorage.app",
            messagingSenderId: "72056385422",
            appId: "1:72056385422:web:b6256ff6c2128d6b5cfedf",
            measurementId: "G-G8ZLNHRTJB"
        };
        ...
        // ============================================
        // ВАШИ ФУНКЦИИ ИЗ ФАЙЛА (теперь они работают)
        // ============================================
        ...
        // Сохранение рекорда
        window.saveScoreFirebase = function(playerName, score) {
            if (!playerName || !score) return;
            
            // Используем имя игрока как ключ
            let playerRef = db.ref('scores/' + playerName);
            
            // Проверяем текущий рекорд и обновляем, только если новый больше
            playerRef.once('value').then(snapshot => {
                let data = snapshot.val();
                let currentScore = data ? data.score : 0;
                
                // Если новый счёт больше — обновляем
        ...
        window.loadLeaderboardFirebase = function(callback) {
            db.ref('scores').once('value')
                .then(snapshot => {
                    ...
                    console.log('Всего загружено:', scores.length, scores);
        ...
        // ============================================
        // ФУНКЦИИ ДЛЯ МУЛЬТИПЛЕЕРА (из вашего файла)
        // ============================================

        // Создание комнаты (Хост)
        // Создание комнаты (Хост) с коротким кодом
        window.createRoomFirebase = function(callback) {
            ...
            function tryCreateRoom(attempt = 0) {
                if (attempt > 10) {
                    console.error('Не удалось создать уникальный код');
                    return;
                }
                ...
                db.ref('rooms/' + roomId).once('value', snapshot => {
                    if (snapshot.exists()) {
                        ...
                        // Создаем комнату с этим кодом
                        db.ref('rooms/' + roomId).set({
                            status: 'waiting',
                            host: { x: 640, y: 320, score: 0, alive: true },
                            guest: { x: 640, y: 320, score: 0, alive: true },
                            createdAt: Date.now()
                        }).then(() => {
        ...
        window.joinRoomFirebase = function(roomId, callback) {
            if (!roomId) return;
        ...
        // Обновление своей позиции (вызывается в OnStep)
        window.updateMyStateFirebase = function(roomId, isHost, bird) {
            if (!roomId || !bird) return;
            
            let role = isHost ? 'host' : 'guest';
            db.ref('rooms/' + roomId + '/' + role).update({
                x: bird.x,
                y: bird.y,
                score: bird.fish || 0,
                alive: !bird.gameover
        ...
        // Прослушивание состояния соперника (вызывается при создании/подключении)
        window.listenToRoomFirebase = function(roomId, isHost, onUpdate) {
            let roomRef = db.ref('rooms/' + roomId);
            roomRef.on('value', snapshot => {
                let data = snapshot.val();
                if (!data) return;
                
                let opponentData = isHost ? data.guest : data.host;
                let status = data.status;
                
                if (onUpdate) {
                    onUpdate(opponentData, status, data);
                }
            });
        };

        // Завершение игры (победа/поражение)
        window.endGameFirebase = function(roomId, winnerName, score) {
            if (!roomId) return;
            ...
            // Сохраняем рекорд победителя
            window.saveScoreFirebase(winnerName, score);
        };
...
// Запускаем загрузку первого скрипта
document.head.appendChild(appScript);
                    }).catch(() => {
...
rooms.templates['MultiplayerMenu'] = {
    ...
    onCreate() {
        /* 🐱👉 room MultiplayerMenu — On room start event (core_OnRoomStart) */
{
templates.withTemplate('bg', function () {
    u.wait(400)
                    .then(() => {
...
rooms.templates['ui2'] = {
    ...
    height: 720,
    behaviors: JSON.parse('[]'),
    objects: JSON.parse('[{"x":638,"y":152,"opacity":1,"tint":16777215,"scale":{"x":0.6,"y":0.4},"rotation":0,"exts":{},"customProperties":{},"template":"gameover"},{"x":0,"y":0,"opacity":1,"tint":16777215,"scale":{"x":1.1962963,"y":1.05555556},"rotation":0,"exts":{},"customProperties":{},"template":"bg"},{"x":510,"y":293,"opacity":1,"tint":16777215,"scale":{"x":0.39915374,"y":0.64135021},"rotation":0,"exts":{},"customProperties":{},"customText":"Создать игру","customSize":"20","template":"createBtn"},{"x":130,"y":380,"opacity":1,"tint":16777215,"scale":{"x":0.39915374,"y":0.64135025},"rotation":0,"exts":{},"customProperties":{},"customText":"Подключиться","customSize":"20","template":"connectBtn"},{"x":499,"y":532,"opacity":1,"tint":16777215,"scale":{"x":0.42877292,"y":0.64135017},"rotation":0,"exts":{},"customProperties":{},"customText":"Назад","template":"backBtn"},{"x":30,"y":332,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"customText":"...","template":"roomIdDisplay"},{"x":7,"y":34,"opacity":1,"tint":16777215,"scale":{"x":2.85361846,"y":2.30952381},"rotation":0,"exts":{},"customProperties":{},"customText":"Морж вмвмы ы ы  ы ы ","customSize":"20","customWordWrap":"130","customAnchor":{"x":0,"y":0},"template":"statusText"},{"x":927,"y":380,"opacity":1,"tint":16777215,"scale":{"x":0.39915374,"y":0.64135021},"rotation":0,"exts":{},"customProperties":{},"customText":"ID","customSize":"18","template":"roomIdInput"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[]'),
    backgroundColor: '#000000',
    ...
    onCreate() {
        /* 🐱👉 room ui2 — On room start event (core_OnRoomStart) */
{
u.wait(770)
                    .then(() => {
                        // ============================================================
// МУЛЬТИПЛЕЕРНОЕ МЕНЮ (комната ui2) — FIREBASE
// ============================================================

// Универсальная функция установки текста
...
// ============================================================
// АДАПТАЦИЯ ВСЕХ UI-ОБЪЕКТОВ (комната ui2)
// ============================================================
let screenW = camera.width;
let screenH = camera.height;
let baseW = 1280;
...
function adaptObject(obj, baseX, baseY, baseScaleX = 1, baseScaleY = 1, tint = null) {
    ...
    obj.x = baseX * scaleX;
    obj.y = baseY * scaleY;
    obj.scale.x = (baseScaleX * scaleX) * 2;
    obj.scale.y = baseScaleY * scaleY;
    if (tint !== null) obj.tint = tint;
}

// Фон
let bg = templates.list['bg']?.[0];
if (bg) {
    bg.anchor.x = 0;
    bg.anchor.y = 0;
    bg.x = 0;
...
// Заголовок
let title = templates.list['gameover']?.[0];
adaptObject(title, 638, 152, 0.6, 0.4);
title.scale.x /= 3

// Кнопки
let createBtn = templates.list['createBtn']?.[0];
let connectBtn = templates.list['connectBtn']?.[0];
let backBtn = templates.list['backBtn']?.[0];
adaptObject(createBtn, 510, 293, 0.39915374, 0.64135021);
adaptObject(connectBtn, 130, 380, 0.39915374, 0.64135025);
adaptObject(backBtn, 499, 532, 0.42877292, 0.64135017);

// Поле ввода ID
let roomIdInput = templates.list['roomIdInput']?.[0];
adaptObject(roomIdInput, 927, 380, 0.39915374, 0.64135021);

// Отображение ID и статуса
adaptObject(templates.list['roomIdDisplay']?.[0], 30, 332, 1, 1);
adaptObject(templates.list['statusText']?.[0], 7, 34, 2.85361846, 2.30952381);

console.log('✅ Все объекты в ui2 адаптированы!');
                    }).catch(() => {
...
rooms.templates['GameMultiplayer'] = {
    name: 'GameMultiplayer',
    width: 1280,
    height: 720,
    behaviors: JSON.parse('[]'),
    objects: JSON.parse('[{"x":1024,"y":320,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"default-player"},{"x":653.47368512,"y":776.07444294,"opacity":1,"tint":16777215,"scale":{"x":2.46491228,"y":1.42910316},"rotation":0,"exts":{},"customProperties":{},"template":"ground"},{"x":192,"y":365,"opacity":1,"tint":16777215,"scale":{"x":0.09,"y":0.09},"rotation":0,"exts":{},"customProperties":{},"template":"Coin"},{"x":249,"y":17,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"score"},{"x":193,"y":38,"opacity":1,"tint":16777215,"scale":{"x":0.09,"y":0.09},"rotation":0,"exts":{},"customProperties":{},"template":"coin_2"},{"x":76,"y":640,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1.15533981},"rotation":0,"exts":{},"customProperties":{},"template":"pipe-bottom"},{"x":76,"y":95,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1.03559871},"rotation":0,"exts":{},"customProperties":{},"template":"pipe-up"},{"x":1280,"y":640,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1.15533981},"rotation":0,"exts":{},"customProperties":{},"template":"pipe-bottom_2"},{"x":1280,"y":95,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1.03559871},"rotation":0,"exts":{},"customProperties":{},"template":"pipe-up_2"},{"x":640,"y":-16,"opacity":1,"tint":16777215,"scale":{"x":40,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"topend"},{"x":0,"y":0,"opacity":1,"tint":16777215,"scale":{"x":1.1851851899999999,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"bg"},{"x":320,"y":320,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"EnemyBird"},{"x":411.77358621,"y":256,"opacity":1,"tint":16777215,"scale":{"x":2.5094339100000003,"y":2.5094339100000003},"rotation":0,"exts":{},"customProperties":{},"template":"VT"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[]'),
    backgroundColor: '#000000',
    
    onStep() {
        /* 🐱👉 room GameMultiplayer — On frame start event (core_OnStep) */
{
// ============================================================
// СИНХРОНИЗАЦИЯ (FIREBASE) — КАЖДЫЙ КАДР
// ============================================================

let myBird = templates.list['default-player']?.[0];
let enemyBird = templates.list['EnemyBird']?.[0];
let roomId = window._roomId;
let isHost = window._isHost;

// ---- ОТПРАВКА СВОЕГО СОСТОЯНИЯ (каждые 3 кадра) ----
if (myBird && roomId) {
    if (Math.floor(u.time * 60) % 3 === 0) {
        window.updateMyStateFirebase(roomId, isHost, myBird);
    }
}

// ---- ОБРАБОТКА СМЕРТИ ----
if (myBird && myBird.gameover && !myBird._deathSent) {
    myBird._deathSent = true;

    // Обновляем свой статус в Firebase
    window.updateMyStateFirebase(roomId, isHost, myBird);

    // Если вы умерли, а соперник ещё жив — вы проиграли
    if (enemyBird && !enemyBird.gameover) {
        window.showDefeat('💀 ВЫ ПРОИГРАЛИ!');
    }
}

// ---- ОБНОВЛЕНИЕ СЧЁТА ----
// if (myBird) {
//     let scoreText = this.getUI('scoreText');
//     if (scoreText) {
//         scoreText.text = 'Счёт: ' + myBird.fish;
//     }
// }
}
    ...
    onCreate() {
        /* 🐱👉 room GameMultiplayer — On room start event (core_OnRoomStart) */
{
fish = 0;
templates.withTemplate('bg', function () {
    u.wait(270)
...
// ============================================================
// ИНИЦИАЛИЗАЦИЯ МУЛЬТИПЛЕЕРНОЙ ИГРЫ (FIREBASE)
// ============================================================

console.log('🎮 Запуск мультиплеерной игры (Firebase)...');
window._gameRoom = this;

let roomId = window._roomId;
let isHost = window._isHost;

if (!roomId) {
    console.log('❌ Нет ID комнаты! Возвращаемся в меню.');
    rooms.switch('ui');
    return;
}

// Объявляем функции победы/поражения ГЛОБАЛЬНО (используем templates.list напрямую)
window.showVictory = function(text) {
    let vt = templates.list['VT']?.[0];
    if (vt) {
        vt.text = text;
        vt.style.fill = '#FFD700';
        vt.style.fontSize = 64;
    }
    let winnerName = N || localStorage.getItem('playerName') || 'Игрок';
    let score = fish || 0;
    window.endGameFirebase(roomId, winnerName, score);
    setTimeout(() => rooms.switch('gameover'), 3000);
};

window.showDefeat = function(text) {
    let vt = templates.list['VT']?.[0];
    if (vt) {
        vt.text = text;
        vt.style.fill = '#FF4444'; // Красный для поражения
        vt.style.fontSize = 64;
    }
    let playerName = N || localStorage.getItem('playerName') || 'Игрок';
    let score = fish || 0;
    window.saveScoreFirebase(playerName, score);
    window.endGameFirebase(roomId, playerName, score);
    setTimeout(() => rooms.switch('gameover'), 3000);
};

// Запускаем прослушивание соперника
window.listenToRoomFirebase(roomId, isHost, function(opponentData, status, data) {
    let enemyBird = templates.list['EnemyBird']?.[0];
    if (enemyBird && opponentData) {
        enemyBird.x = opponentData.x;
        enemyBird.y = opponentData.y;
        enemyBird.fish = opponentData.score;

        if (opponentData.alive === false) {
            enemyBird.gameover = true;
            enemyBird.alpha = 0.3;
            let myBird = templates.list['default-player']?.[0];
            if (myBird && !myBird.gameover) {
                window.showVictory('🏆 ВЫ ПОБЕДИЛИ!');
            }
        }
    }
    ...
    if (status === 'finished') {
        ...
        let myName = N || localStorage.getItem('playerName') || 'Игрок';
        if (winner !== myName) {
            window.showDefeat('💀 ВЫ ПРОИГРАЛИ!');
        }
    }
...
rooms.templates['toplistRoom'] = {
    name: 'toplistRoom',
    width: 1280,
    height: 720,
    behaviors: JSON.parse('[]'),
    objects: JSON.parse('[{"x":0,"y":0,"opacity":1,"tint":16777215,"scale":{"x":1.18519,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"bg"},{"x":17,"y":54,"opacity":1,"tint":16777215,"scale":{"x":1,"y":0.5},"rotation":0,"exts":{},"customProperties":{},"customText":"...","template":"leaderboardText"},{"x":977,"y":0,"opacity":1,"tint":16777215,"scale":{"x":0.428,"y":0.641},"rotation":0,"exts":{},"customProperties":{},"customText":"Назад","template":"backBtn2"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[]'),
    backgroundColor: '#000000',
    ...
    onCreate() {
        /* 🐱👉 room toplistRoom — On room start event (core_OnRoomStart) */
{
templates.withTemplate('bg', function () {
    u.wait(400)
                    .then(() => {
...
let leaderboardText = templates.list['leaderboardText']?.[0];
adaptObject(leaderboardText, 17, 54, 1, 0.5);

let room = this || rooms.current;

// ==== ЗАГРУЗКА ЛИДЕРБОРДА В UI ====
window.loadLeaderboardFirebase(function(scores) {
    let textElement = templates.list['leaderboardText']?.[0]; // или с фильтром по комнате
    if (textElement) {
        let text = '🏆 ЛУЧШИЕ ИГРОКИ\n━━━━━━━━━━━━━━━━━━\n';
        if (scores.length === 0) {
            text += 'Пока нет рекордов...\nСтань первым! 🎮';
            ...
            scores.forEach((entry, index) => {
                let medal = ['🥇', '🥈', '🥉'][index] || (index + 1) + '.';
                text += medal + ' ' + (entry.name || 'Аноним') + ': ' + entry.score + '\n';
            });
        }
        textElement.text = text;
        console.log('Текст установлен:', textElement.text); // <-- ДОБАВЛЕНО
    }
});
